...
def test(args, net, test_loader, textio):
    ...
    net.eval()
    if args.test:
        model_path = "checkpoints/dcp/models/model.best.t7"
...
def train_one_epoch(args, net, train_loader, opt):
    ...
    for src, target, rotation_ab, translation_ab in tqdm(train_loader):
        ...
        translations_ab.append(translation_ab.detach().cpu().numpy())
        rotations_ab_pred.append(rotation_ab_pred.detach().cpu().numpy())
        angle_ab_pred = Rotation.from_matrix(rotation_ab_pred.detach().cpu().numpy())
...
def train(args, net, train_loader, test_loader, textio, boardio):
    ...
    for epoch in range(args.epochs):
        ...
        if best_test_loss >= test_loss:
            ...
            torch.save(net.state_dict(), 'checkpoints/dcp/models/model.best.t7')
        ...
        if epoch % 10 == 0:
            torch.save(net.state_dict(), 'checkpoints/dcp/models/model.%d.t7' % epoch)
        scheduler.step()
...
if __name__ == '__main__':
    ...
    boardio = SummaryWriter(logdir='checkpoints/dcp')
    textio = open('checkpoints/dcp/run.log', 'a')
